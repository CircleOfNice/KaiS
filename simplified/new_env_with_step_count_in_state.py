...
class Master:
    ...
    def __init__(self, number_of_nodes, data, train = True):
        ...
        shape = self.get_master_observation_space().shape
        self.observation_space_dims = shape[0], shape[1]+1
        self.data = data
...
class CustomEnv(gym.Env):
  ...
  def reset(self):
    self.master.reset()
    observation = self.get_new_state()
    self.step_counter = 0
  ...
  def get_new_state(self):
    ...
    #observation = np.random.rand(self.number_of_nodes,self.number_of_nodes)
    observation = np.ones((self.number_of_nodes,2+1))#*(self.step_counter/self.data_len)
    observation[:,:-1] = observation_
    return observation
  def step(self, action):
    ...
    #if reward >0:
    reward = reward#*(self.step_counter/ self.data_len)#*(1-(self.step_counter/ self.data_len))#- self.step_counter*self.data_len
    #print('self.step_counter, reward : ', self.step_counter, reward)
    ...
    #print(self.step_counter)
        ...
        #self.update_incoming_task(task) 
    #    self.master.reset()
        #print('master reset: ', self.step_counter )
